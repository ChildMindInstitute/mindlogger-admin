...
export const getConditionMinMaxValues = ({ item, state }: { item?: ConditionItem; state: ConditionType }) => {
  if (!item?.type) return getDefaultMinMaxValues(state);

  switch (item.type) {
    case ConditionItemType.Slider:
      return {
        minNumber: state ? +(item.responseValues! as SliderItemResponseValues).minValue : DEFAULT_NUMBER_MIN_VALUE,
        maxNumber: state ? +(item.responseValues! as SliderItemResponseValues).maxValue : Number.MAX_SAFE_INTEGER,
      };
    default:
