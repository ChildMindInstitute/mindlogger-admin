...
describe('SaveAndPublishProcessPopup', () => {
  ...
  test('renders correctly for a beingCreated', () => {
    renderWithProviders(<SaveAndPublishProcessPopup step={SaveAndPublishSteps.BeingCreated} {...commonProps} />);

    expect(
      screen.getByTestId(`builder-save-and-publish-popup${saveAndPublishProcessTestIds.beingCreated}`),
    ).toBeInTheDocument();
  });

  test('renders correctly for a reportConfigSave', () => {
    renderWithProviders(<SaveAndPublishProcessPopup step={SaveAndPublishSteps.ReportConfigSave} {...commonProps} />);

    expect(
      screen.getByTestId(`builder-save-and-publish-popup${saveAndPublishProcessTestIds.reportConfigSave}`),
    ).toBeInTheDocument();
  });

  test('calls onClose when the close button is clicked', () => {
    renderWithProviders(<SaveAndPublishProcessPopup step={SaveAndPublishSteps.Failed} {...commonProps} />);
