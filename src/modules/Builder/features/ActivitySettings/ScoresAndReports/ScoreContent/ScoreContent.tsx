...
export const ScoreContent = React.memo(
  ({
    name,
    title,
    index,
    ...
    // Use selective watching for only the specific score instead of entire arrays
    ...
    // Get subscales only when needed instead of watching the entire array
    const eligibleSubscales = useMemo(() => {
      const subscales: SubscaleFormValue[] = getValues(subscalesField) ?? [];

      return subscales.filter(({ subscaleTableData, items }) => {
    ...
    }, [getValues, subscalesField, scoreItems]);
