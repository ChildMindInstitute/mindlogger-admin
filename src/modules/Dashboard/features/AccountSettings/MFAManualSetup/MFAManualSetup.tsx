...
export const MFAManualSetup = ({ open, onClose, onBack, secretKey }: MFAManualSetupProps) => (
  <StyledDialog open={open} onClose={onClose} maxWidth={false}>
    ...
    <StyledContent>
      ...
      <StyledDescriptionMultiLine>
        Then, enter the verification code that appears in your authenticator app to complete setup.
      </StyledDescriptionMultiLine>

      <Box sx={{ width: '300px', position: 'relative' }}>
        <StyledInputContainer hasError={false}>
          <StyledInput type="text" inputMode="numeric" placeholder="Enter verification code" />
        </StyledInputContainer>
      </Box>

      <StyledButtonContainer>
        <StyledButton type="button" className="primary">
          Continue
        </StyledButton>
        <StyledButton type="button" className="secondary" onClick={onBack}>
          Back
        </StyledButton>
      </StyledButtonContainer>
    </StyledContent>
