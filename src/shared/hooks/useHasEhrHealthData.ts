...
export const useHasEhrHealthData = ({
  ...
  return useMemo(() => {
    // Only proceed if the feature flag is active
    if (featureFlags.enableEhrHealthData !== 'active') return false;

    // If no applet data is available, return false
    if (!data?.activitiesDetails) return false;

    const activities = data?.activitiesDetails ?? [];
    const flows = data?.appletDetail.activityFlows ?? [];

    // Determine which activities to check based on the provided parameters
