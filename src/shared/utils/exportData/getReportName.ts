...
export const getStabilityTrackerCsvName = (id: string, phaseType: DecryptedStabilityTrackerAnswerObject['phaseType']) =>
  `${id}_${phaseType}.csv`;

export const getABTrailsCsvName = (index: number, id?: string) => `${id || ''}-trail${index + 1}.csv`;
...
export const getFlankerCsvName = (item: DecryptedAnswerData) => `${item.id}-${item.activityItem.name}.csv`;
